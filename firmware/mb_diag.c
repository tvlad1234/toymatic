...
{
    static uint32_t next_diag_query = 0;
    if (mb->state == MB_IDLE) // Only if we aren't awaiting response or are busy
    {
        if ((int32_t)(time_us - next_diag_query) >= 0)
        {
            ...
            mb->state = MB_AWAIT_RESPONSE;
            next_diag_query += 10000;
        }
    }
    else if (mb->state == MB_AWAIT_RESPONSE)
    {
        // Timeout after 250ms ms of not having a response
        if ((int32_t)(time_us - next_diag_query) >= 0)
        {
            mb->state = MB_IDLE;
            next_diag_query += 250000;
        }
    }
    else if (mb->state == MB_PROCESSING)
    {
